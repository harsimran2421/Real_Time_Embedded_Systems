//Modifications made in Dr Sam Stiewert's Code
#include <pthread.h>
#include <stdio.h>
#include <sched.h>
#include <time.h>
...
void *grabRsrcs(void *threadid)
{
  while(1){
   ...
   if((intptr_t)threadid == THREAD_1) printf("Thread 1 started\n");
   else if((intptr_t)threadid == THREAD_2) printf("Thread 2 started\n");
   ...
   if((intptr_t)threadid == THREAD_1)
   {
     ...
     else if(rc == ETIMEDOUT)
     {
         ...
         sleep((rand()%4)+1);            // Random back off
         continue;
         //pthread_exit(NULL);
     }
     else
