...
void Schedule_process(uint32_t Number_of_services)
{
  ...
  for(j = 0; j<Number_of_services; j++ )
  {
    if(nextDeadline[j]<900)
    {
      nextDeadline[j] = ProcessesPeriods[j] - ((j)%ProcessesPeriods[j]) - Remaining_capacity[j];
    }
   }

  if(Remaining_capacity[EDIndex] == 0)
  {
    nextDeadline[EDIndex] = 950;
  }
